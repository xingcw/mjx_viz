...
def scan_ckpt_runs(root: str | Path) -> list[dict]:
    ...
    for entry in sorted(root_p.iterdir()):
        ...
        if suffix is None:
            continue
        eval_pkl = _first_existing(entry, _EVAL_PKL_NAMES)
        train_done = _first_existing(entry, _TRAIN_DONE_NAMES) is not None
        eval_done_marker = _first_existing(entry, _EVAL_DONE_NAMES) is not None
        runs.append(
            {
                "name": entry.name,
                "path": str(entry),
                "run_suffix": suffix,
                "morphology_metadata": _read_json(entry / "morphology_metadata.json"),
...
def load_reward_curve(run_path: str | Path) -> dict | None:
    ...
    """
    run_p = Path(run_path)
    eval_pkl = _first_existing(run_p, _EVAL_PKL_NAMES)
    if eval_pkl is None:
        return None
...
def load_final_rewards(root: str | Path) -> dict[str, float]:
    """Return {run_name: final_reward} for every run with an eval pkl."""
    out: dict[str, float] = {}
    for info in scan_ckpt_runs(root):
        if info["eval_curve_path"] is None:
            continue
        curve = load_reward_curve(info["path"])
